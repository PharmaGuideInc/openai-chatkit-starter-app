...
export function ChatHistorySidebar({
  ...
  return (
    <aside className="cpsgo-sidebar flex h-full w-full md:w-72 shrink-0 flex-col border-r border-slate-200 bg-gray-50">
      {/* Mobile header with back button */}
      <div className="flex items-center gap-3 p-4 border-b border-slate-200 md:hidden">
        <button
          type="button"
          onClick={onCloseMobile}
          className="flex h-9 w-9 items-center justify-center rounded-lg hover:bg-slate-100 cursor-pointer"
        ...
        <h1 className="flex-1 text-lg font-medium text-slate-900">Chat history</h1>
      </div>

      {/* Desktop header with icons */}
      <div className="hidden md:flex items-center justify-end p-3 border-b border-slate-200">
        <button
          type="button"
          onClick={onNewChat}
          className="flex h-8 w-8 items-center justify-center rounded-lg hover:bg-slate-200 cursor-pointer"
          aria-label="New chat"
        >
      ...
      <div className="p-4 md:hidden">
        <button
          type="button"
          onClick={onNewChat}
          className="flex w-full items-center gap-3 rounded-lg border px-4 py-3 text-left cursor-pointer"
          style={{ borderColor: 'var(--cpsgo-primary)' }}
        >
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5" style={{ color: 'var(--cpsgo-primary)' }}>
            <path d="M21.174 6.812a1 1 0 0 0-3.986-3.987L3.842 16.174a2 2 0 0 0-.5.83l-1.321 4.352a.5.5 0 0 0 .623.622l4.353-1.32a2 2 0 0 0 .83-.497z" />
            <path d="m15 5 4 4" />
          </svg>
          <span className="text-base font-medium text-slate-900">New chat</span>
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-2">
        ...
        {sorted.length === 0 ? (
          ...
          <>
            <h3 className="px-3 py-2 text-xs font-semibold text-slate-600">Earlier</h3>
                ...
                <li key={t.id} className="relative">
                  {editingId === t.id ? (
                    <div className={
                      ...
                      <input
                        className="w-full rounded-md border border-slate-300 px-2 py-1 text-slate-900 outline-none bg-white"
                        autoFocus
                        value={editingTitle}
                      ...
                      <div className="mt-2 flex gap-2">
                        <button
                          ...
                          className="rounded-md px-2 py-1 text-xs text-white disabled:opacity-50 cursor-pointer"
                        ...
                        <button
                          ...
                          className="cpsgo-btn-secondary rounded-md px-2 py-1 text-xs cursor-pointer"
                        >
                          Cancel
                        </button>
                      </div>
                    ...
                    <div className={
                      "cpsgo-sidebar-item group flex items-center gap-1 rounded-md transition cursor-pointer " +
                      (isActive
                        ? "bg-slate-200"
                        : "hover:bg-slate-100 text-slate-700")
                    }>
                      ...
                      {menuOpenFor === t.id ? (
                        <div
                          className="absolute right-2 top-9 z-20 w-32 rounded-md border border-slate-200 bg-white p-1 text-sm shadow-lg"
                          onClick={(e) => e.stopPropagation()}
                          role="menu"
                        >
                        <button
                          type="button"
                          className="block w-full rounded px-2 py-1 text-left cursor-pointer transition-colors"
                          style={{ color: 'var(--cpsgo-primary)' }}
                          onMouseEnter={(e) => e.currentTarget.style.backgroundColor = 'rgba(82, 184, 197, 0.1)'}
                        ...
                        <button
                          type="button"
                          className="mt-1 block w-full rounded px-2 py-1 text-left text-red-600 hover:bg-red-50 cursor-pointer"
